...
static bool files_to_sites( const char* name, const ArgumentList &arguments,EvalState & state,Value &result);
                            ...
                            std::vector<std::string> &result)
{
  ...
  for (int i=0; i<arguments.size(); i++) {
    std::string s = std::string(arguments[0]);
  ...
  //return bestHostSites[0].c_str();
  result = bestHostSites;
  return true;
}
